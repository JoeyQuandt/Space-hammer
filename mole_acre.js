AFRAME.registerComponent("control", {
  ...
  init: function () {
    ...
    if (
      ...
      this.cursor.setAttribute(
        "animation__click",
        "property: rotation; startEvents: click; easing: easeInCubic; dur: 250; from: -90 0 0; to: 0 0 0"
      );
      this.cursor.setAttribute(
        "animation__fusing",
        "property: scale; startEvents: fusing; easing: easeInCubic; dur: 500; from: 0.1 0.1 0.1; to:0.20 0.20 0.20"
      );
      this.cursor.setAttribute(
        "animation__mouseleave",
        "property: scale; startEvents: mouseleave; easing: easeInCubic; dur: 250; to: 0.1 0.1 0.1"
      );
      this.cursor.setAttribute(
        "geometry",
    ...
    this.el.addEventListener(this.event, () => {
      if (this.event == "click" || this.collidable) {
        ...
        this.controlInstructions.setAttribute(
          "text",
          "value",
          `Time: ${this.duration}`
        );
  ...
  displayHighscore: function () {
    this.highscoreText.setAttribute(
      "text",
      "value",
  ...
  allowRestart: function () {
    ...
    this.el.addEventListener("animationcomplete", (e) => {
      //listen for when thee control entity has been animated back to it initial position
...
AFRAME.registerComponent("mole-logic", {
  init: function () {
    ...
    el.addEventListener("collidestart", (e) => {
      if (this.collidable) {
        ...
        this.el.setAttribute(
          "ammo-body",
          "collisionFilterGroup: 30; collisionFilterMask: 20"
        ); //do not detect collision with hammer
        el.object3D.rotation.x = this.hitRotation;
        this.hit = true;
        this.collidable = false;
      }
    ...
    el.addEventListener("visible", (e) => {
      ...
      this.el.setAttribute(
        "ammo-body",
        "collisionFilterGroup: 1; collisionFilterMask: 5"
      ); //detect collision with hammer
    });
    ...
    el.addEventListener("animationcomplete", (e) => {
      ...
      document
        ...
        .emit("setAsWhackable", { entity: e.target });
      this.el.setAttribute(
        "ammo-body",
        "collisionFilterGroup: 30; collisionFilterMask: 20"
      );
  ...
  init() {
    ...
    this.el.addEventListener("enable", () => {
      ...
      this.el.addEventListener("hammerBack", () => {
        ...
        this.el.setAttribute(
          "ammo-body",
          "collisionFilterGroup: 4; collisionFilterMask: 7"
        ); //allow collisions with inhand hammer
      ...
      this.flyingHammer.addEventListener("collidestart", (e) => {
        ...
        this.topRibbon.setAttribute(
          "mixin",
          "hammer-ribbon ribbon-recall-animation"
        );
      });
        ...
        if (e.srcElement == this.el.parentElement) {
          {
            if (this.hammerState == this.states.SHOT) {
              ...
              this.bottomRibbon.setAttribute("mixin", "hammer-ribbon");
              this.bottomRibbon.setAttribute("position", "0 -0.14 0");
            } else if (this.hammerState == this.states.INHAND) {
              //if the hammer is inhand create a flying hammer and hide controller hammer
              ...
              this.flyingHammer.setAttribute(
                "position",
                "" +
                  lastPosition.x +
                  " " +
              ...
              this.el.setAttribute(
                "ammo-body",
                "collisionFilterGroup: 10; collisionFilterMask: 10"
              );
              this.bottomRibbon.setAttribute(
                "mixin",
                "hammer-ribbon ribbon-shot-animation"
              );
              this.el.emit("playHammerSound");
            }
          }
...
AFRAME.registerComponent("flying-hammer", {
  ...
  calcDistance() {
    //between controller hammer and flying hammer
    let origin = this.createPositionData();
  ...
  setRecallAnimation(pos) {
    ...
    this.el.setAttribute(
      "animation__recall" + this.animationCounter,
      `property:position; to:${pos}; dur: ${dur};`
    );
  },
